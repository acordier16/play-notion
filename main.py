...
def build_headers_and_json(notion_token, tags):
    ...
    if not "set" in tags: # If "set" tag was not selected, specifically avoid it.
        and_query.append({"property": "Tags", "multi_select": {"does_not_contain": "set"}})
    json = {
...
def get_existing_tags(response):
    ...
    existing_tags = [item["name"] for item in response.json()["properties"]["Tags"]["multi_select"]["options"]]
...
def get_tracks_urls(response, random, tracks_number):
    ...
    # The & character is a problem when passing the command to subprocess,
    # so we need to escape it for each URL
...
def build_mpv_command(urls, window):
    command = "mpv --term-playing-msg='Title: ${media-title}' --force-seekable=yes --script-opts=ytdl_hook-ytdl_path=" + YTDLP_PATH
    if window:
...
def main():
    parser = argparse.ArgumentParser(description="Plays music from a tagged Notion database via yt-dlp and mpv. Will retrieve the most recent tracks first unless specified. This script requires yt-dlp and mpv to be installed.")
    parser.add_argument("tags", type=str, nargs="+", help="A list of queried tags. Tags will be queried in an 'AND' fashion. By default, tracks with tag 'set' will not be retrieved (unless specified). Default: 'techno'", default="techno")
    parser.add_argument("-n", "--number", type=int, help="The number of tracks to retrieve and play. Default: 30", default=30)
    parser.add_argument("-r", "--random", action='store_true', help="If present, tracks will be retrieved in a 'random' order rather than in a 'most recent first' one.")
    parser.add_argument("-w", "--window", action='store_true', help="If present, will open a GUI window for mpv.")
    ...
    existing_tags = make_request_and_do(url=NOTION_DATABASE_URL, headers=headers, json=None, do=get_existing_tags)

    # Query the database to get tracks URLs
    urls = make_request_and_do(url=f"{NOTION_DATABASE_URL}/query", headers=headers, json=json, do=partial(get_tracks_urls, random=args.random, tracks_number=args.number))
